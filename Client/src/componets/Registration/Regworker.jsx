...
export default function Regworker() {
   ...
   console.log(input);
    if(file){
      const data =new FormData()
        ...
        axios.post('http://localhost:4000/register/upload-image',data).then((response)=>{
      console.log(response);
    })

    }
    axios.post('http://localhost:4000/register/Regworker',input).then((response)=>{
      ...
      <div className="col-lg-7 mb-5 mb-lg-0">
        <div className="contact-form">
          ...
          <form name="sentMessage" id="contactForm" noValidate="novalidate">
            ...
            <span style={{color:'red'}}>{formErrors?.contct}</span>
              <input
                type="text"
                className="form-control p-4"
                id="subject"
            ...
            <div className=" control-group">
                <input
                  ...
                  required="required"
                  data-validation-required-message="categorywork"
                  name="categorywork"
                  onChange={inputChange}
                />
                <p className="help-block text-danger" />
              ...
              <div className=" control-group">
             <h8>Upload your profile photo</h8>
                <input
                  type="file"
                  className="form-control p-4"
                  id="file"
                  ...
                  name="workerimage"
                  onChange={(Event)=> { setFile(Event.target.files[0]); setInput({...input, workerimage: Event.target.files[0].name})}}
                />
                <p className="help-block text-danger" />
              </div>
