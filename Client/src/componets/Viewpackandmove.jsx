...
export default function Viewpackandmove() {

  const navigate = useNavigate()
    const [packandmove, setPackandmove] = useState([])
    console.log(packandmove);
    useEffect(() => {
      axios.get('http://localhost:4000/admin/viewpack').then((response) => {
          // console.log("response====>", response.data.details);
          setPackandmove(response.data.details)
      }).catch((err) => {
          console.log(err);
      })
  }, [])
  const approve = (id) => {
    ...
    axios.get(`http://localhost:4000/admin/approve-pack/${id}`).then((response) => {
   ...
   const reject = (id) => {
    ...
    axios.get(`http://localhost:4000/admin/reject-pack/${id}`).then((response) => {
     console.log(response);
     toast.success(response.data.message, {
      position: "top-center",
      autoClose: 5000,
